...
// category array with All option
const categories = menu.reduce((total, current) => {
...
categories.forEach((item) => {
  const button = `
  <button class="btn btn-outline-dark btn-item" data-cat="${item}">
    ${item}
  </button>
  `
...
function renderMenu(menuArray) {
  ...
  menuArray.forEach(item => {
    const card = `
      ...
      <div class="menu-text menu-origin">Origin: ${item.category}</div>
      </div>
      </div>
      `
    menuDOM.innerHTML += card;
  })
}
...
buttons.forEach((button) => {
  button.addEventListener("click", (e) => {
    ...
    if (cat == "All") {
      ...
      const temp = menu.filter(item => item.category == cat)
      renderMenu(temp)
    }
  })
})
